...
public class Intervals {
	...
	public static double goldenRatio(Point x0, double a, double b, boolean isUnimodal, Point v, int currentDimension, Function f) {
		...
		if(!isUnimodal) {
			double[] arr = unimodalSearch(h, x0, v, currentDimension, f);
			a = arr[0];
			b = arr[1];
		}
...
//		System.out.println(String.format("%.3f %.3f %.3f %.3f", a, c, d, b));
	...
	public static double[] unimodalSearch(double h, Point x0, Point v, int currentDimension, Function f) {
		int step = 1;
		double lambda = x0.getValue(currentDimension);
		double m = lambda;
